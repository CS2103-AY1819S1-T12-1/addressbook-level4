...
Format: `find [n/NAME] AND[OR] [c/CATEGORY] AND[OR] [c/COST] AND[OR] [t/TAG] AND[OR] [d/DATE:DATE]`

Examples:

* find c/School
Find all the expenses under `School` category.
* find c/Food AND t/Lunch
Find all the expenses with tag `Lunch` under `Food` category.
* find c/Food OR t/Lunch
...
* find c/>10.00
Find all the expenses with cost larger than 10.00.

****
* The search is case insensitive. e.g `school` will match `School`
* The order of the keywords does not matter. e.g. `c/Food t/Lunch` will match `t/Lunch c/Food`
* Only full words will be matched e.g. `sc` will not match `school`
